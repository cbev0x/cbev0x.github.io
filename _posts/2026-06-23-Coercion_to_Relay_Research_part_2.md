...
- The CA's web enrollment role (`certsrv`), when installed on Server 2025, is configured to require SSL by default, but doesn't auto-provision a certificate or HTTPS binding, so it's effectively non-functional until an administrator adds one. This is a more defensible default posture than plain HTTP being open out of the box; the gap is operational (no working HTTPS endpoint without setup) rather than a security one (no protection on an exposed HTTP endpoint). NTLM over plain HTTP still has no equivalent to LDAP signing at all, so that path remains fully unprotected on any build or configuration where HTTP enrollment is actually enabled.
- Whether EPA is enabled by default has changed across Windows Server versions and is worth verifying directly rather than assuming. On Server 2025, a fresh AD CS Web Enrollment install sets IIS's `extendedProtection` to `tokenChecking="Require"` immediately after role installation, with no manual configuration. This is a meaningful shift from older guidance (including Microsoft's own KB5005413), which has historically treated EPA as something an administrator must explicitly turn on. Whether this default holds for upgraded CAs running on older OS versions, or only fresh Server 2025 installs, is a separate question worth its own verification; the EPA-by-default behavior described here was confirmed in Part 4 against a clean Server 2025 build specifically.
